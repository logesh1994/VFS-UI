...
-- -----------------------------------------------------
-- Table `outreach_vfs`.`event_benificiary`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `outreach_vfs`.`event_benificiary` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `description` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id`));
...
CREATE TABLE IF NOT EXISTS `outreach_vfs`.`event` (
  ...
  CONSTRAINT `Event_fk1`
    ...
    REFERENCES `outreach_vfs`.`event_benificiary` (`id`),
  CONSTRAINT `Event_fk2`
    FOREIGN KEY (`event_poc_id`)
    REFERENCES `outreach_vfs`.`employee` (`id`),
...
CREATE TABLE IF NOT EXISTS `outreach_vfs`.`feedback` (
  ...
  `email_feedback_status` INT(11) NOT NULL,
  ...
  CONSTRAINT `Feedback_fk3`
    FOREIGN KEY (`email_feedback_status`)
    REFERENCES `outreach_vfs`.`status` (`id`),
  CONSTRAINT `Feedback_fk4`
...
CREATE INDEX `Feedback_fk3` ON `outreach_vfs`.`feedback` (`email_feedback_status` ASC);

CREATE INDEX `Feedback_fk4` ON `outreach_vfs`.`feedback` (`update_id` ASC);